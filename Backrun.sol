...
contract Backrun {
    ...
    */
    function updateTokenQuantities(euint64 amountIn, euint64 value, bytes memory methodID, euint64 EthInPool, euint64 USDTInPool) public view returns (euint64, euint64, bytes memory){
        ...
        if (methodID[0] == 0x18){ 
            // Tokens For ETH  
            searchersMethodID = hex"7ff36ab5"; // methodID of the searcher's transaction
            ...
            euint64 newEthInPool = TFHE.mul(EthInPool, 100000000); // to avoid 
            ...
            // Eth For Tokens 
            searchersMethodID = hex"18cbafe5"; // methodID of the searcher's transaction
            euint64 newValue = TFHE.div(value, 1000000000000);  // To align the precision to USDT
            euint64 uniswapFees = TFHE.div(TFHE.mul(newValue, 3), 1000);
            euint64 amountIn_fees = TFHE.sub(newValue, uniswapFees); // how much ETH was actually traded in 
            euint64 token1AfterSwap = TFHE.add(EthInPool, amountIn_fees); 
            token2 = TFHE.add(EthInPool, newValue); // eth after swap  
            euint64 newEthInPool = TFHE.mul(EthInPool, 100000); // x 1000000 to help with int division 
            token1 = TFHE.div(newEthInPool,TFHE.decrypt(token1AfterSwap)); // usdt after swap 
            token1 = TFHE.div(TFHE.mul(token1, USDTInPool), 100000); // undoing earlier multiplication   
        }
    ...
    */
    function amountCalculation(Main.ProfitConstants memory searcherConstants, euint64 X, euint64 Y) public view returns(euint64 eamountIn){
        X = TFHE.mul(X, 1000000); // x * precision 
        euint64 temp = TFHE.sub(TFHE.mul(TFHE.mul(searcherConstants.encryptedFour, searcherConstants.maxBuyPrice), Y), TFHE.div(TFHE.mul(12, TFHE.mul(searcherConstants.maxBuyPrice, Y)), 1000)); //  (4*price*y*(1-fee))
        euint64 radicand = TFHE.mul(X, TFHE.add(TFHE.div(TFHE.mul(9,X), 1000000), temp)); //radicand
        euint64 eamountDividend = TFHE.sub(TFHE.add(esqrt(radicand), TFHE.div(TFHE.mul(X, 3), 1000)) , TFHE.mul(2, X));  
        eamountIn = TFHE.div(eamountDividend , 1994000); // divisor calculation
        return eamountIn;
    }
    ...
    /*
        ...
        *
    */
    function calculateProfits(euint64 X, euint64 Y, euint64 eamountIn, Main.ProfitConstants memory searcherConstants) public view returns(euint64 profit, euint64){
        euint64 x_after_fee;
        euint64 y_after;
        x_after_fee = TFHE.sub(TFHE.add(X, eamountIn) ,TFHE.div(TFHE.mul(eamountIn, 3), 1000)); // searcher trades in Eth
        uint64 decoded_x =  TFHE.decrypt(x_after_fee);
        y_after = TFHE.div(TFHE.mul(X,Y), decoded_x); // USDT after
        profit = TFHE.div(TFHE.mul(TFHE.sub(searcherConstants.minSellPrice, searcherConstants.maxBuyPrice), TFHE.sub(Y, y_after)), 1000000);
    ...
    /*
        ...
        * Author: OpenZeppelin
        * Title: Math.sol
        * URL: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/math/Math.sol
        *
